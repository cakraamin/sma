...
class Mmapel extends CI_Model{
	...
	function getMapelKelase($id)
	{
		...
		{
			$sql = "SELECT c.id_mapel_kelas,a.mapel,d.nama FROM mapel a,guru_mapel b,guru_mapel_kelas c,guru d WHERE a.id_mapel=b.id_mapel AND b.id_guru_mapel=c.id_guru_mapel AND c.id_kelas='$id' AND a.jenis='$dt_jenis' AND b.nip=d.nip";
			$kueri = $this->db->query($sql);			

			$hasil[$dt_jenis] = $kueri->result_array();				
			//$hasil[$dt_jenis] = $sql;				
		}
	...
	public function addGuruKelas($kelas)
	{
		$data = array(
		   'id_mapel_kelas' => '',
		   'id_guru_mapel' 	=> $this->input->post('nip',TRUE),
		   'id_kelas' 		=> $kelas,
	...
	public function updateGuruKelas($id)
	{
		$data = array(		  
		   'id_guru_mapel' 	=> $this->input->post('nip',TRUE),		   
		);
	...
	{
		$kueri = $this->db->query("INSERT INTO guru_mapel_kelas(id_guru_mapel,id_kelas,id_ta) VALUES('$id','".$this->input->post('kelas',TRUE)."','".$this->session->userdata('kd_ta')."')");
		return $kueri;
	}
	...
	function getMapelGuruKelas($id,$num,$offset,$sort_by,$sort_order)
	{
		...
		$sql = "SELECT a.id_mapel_kelas,b.nama,b.id_kelas,b.tingkat,b.kode,b.jenis,b.id FROM guru_mapel_kelas a,kelas b,guru_mapel c WHERE a.id_kelas=b.id_kelas AND a.id_guru_mapel=c.id_guru_mapel AND c.id_guru_mapel='$id' GROUP BY a.id_kelas ORDER BY $sort_by $sort_order LIMIT $offset,$num";
		$query = $this->db->query($sql);
		return $query->result();
	}
	...
	{
		$sql = "SELECT a.id_mapel_kelas,b.nama,b.id_kelas FROM guru_mapel_kelas a,kelas b,guru_mapel c WHERE a.id_kelas=b.id_kelas AND a.id_guru_mapel=c.id_guru_mapel AND c.id_guru_mapel='$id' ";
		$query = $this->db->query($sql);
		return $query->num_rows();
	}
